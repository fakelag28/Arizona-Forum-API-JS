...
function toNumbers(value) {
    ...
    value.replace(/(..)/g, (match) => {
        numbers.push(parseInt(match, 16));
    });
    return numbers;
}
...
async function bypassAntibot(userAgent, proxy = null) {
    let axiosConfig = {
        headers: {
    ...
    try {
        const response = await axios.get('https://forum.arizona-rp.com/', axiosConfig);
        const html = response.data;

        const codes = html.split(',\"\\x30\",\"\\x74\\x6F\\x4C\\x6F\\x77\\x65\\x72\\x43\\x61\\x73\\x65\",')[1]
            .split(',\"\\x63\\x6F\\x6F\\x6B\\x69\\x65\",')[0];
        
        const found = codes.match(/"(.*)","(.*)","(.*)"/);
        if (!found) {
            throw new Error('Failed to extract antibot codes');
        }

        const a = toNumbers(found[1]);
        const b = toNumbers(found[2]);
        const c = toNumbers(found[3]);
        ...
        vm.run(text);
        const decrypted = vm.run('slowAES.decrypt(array[0], 2, array[1], array[2])');
        
        const antibotValue = _0xfab6[11] + toHex([decrypted, _0xfab6]);

        return {
            cookie: antibotValue + _0xfab6[13],
            userAgent: userAgent
        };

    } catch (error) {
        throw new Error(`Error bypassing antibot: ${error.message}`);
    }
}
...
async function main() {
    ...
    try {
        const result = await bypassAntibot(userAgent);
        console.log('Antibot bypass successful:');
        console.log('Cookie:', result.cookie);
        console.log('User-Agent:', result.userAgent);
    } catch (error) {
        console.error('Failed to bypass antibot:', error.message);
    }
}

module.exports = { bypassAntibot };
